...
Resample to daily values by chosing the maximum value on each day.
...
Or, chose the minimum value on each day.
...
ax.legend(["Original", "Upsampled (mean)", "Upsampled (min)", "Upsampled (max)"])
ax.grid(which="both")
```
...
Compare the difference between these two. Find the new time stamps and how they're values were chosen.
